"""human-verify OCR queue: new invoice/audit enum values

Adds NEEDS_VERIFICATION to invoice_status and two audit event types for the
verification flow (TD-3). On Postgres these are native enum types and require
ALTER TYPE ... ADD VALUE; on SQLite the Enum is inlined as a VARCHAR (no
separate type object), so there is nothing to migrate there.
...
_NEW_INVOICE_STATUS_VALUES = ("needs_verification",)
_NEW_AUDIT_EVENT_TYPES = (
    "invoice_needs_verification",
    "invoice_verified",
)

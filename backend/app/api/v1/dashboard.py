...
def get_overview(current_user: CurrentUser, db: DBSession):
    ...
    return DashboardService(db, current_user.tenant_id).get_overview()
...
def invoice_summary(current_user: CurrentUser, db: DBSession):
    ...
    return DashboardService(db, current_user.tenant_id)._invoice_summary()
...
def discrepancy_summary(current_user: CurrentUser, db: DBSession):
    ...
    return DashboardService(db, current_user.tenant_id)._discrepancy_summary()
...
def queue_status(current_user: CurrentUser, db: DBSession):
    ...
    return DashboardService(db, current_user.tenant_id)._queue_status()
...
def analytics_trends(current_user: CurrentUser, db: DBSession):
    ...
    return DashboardService(db, current_user.tenant_id)._recent_trends()
